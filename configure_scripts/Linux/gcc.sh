...
GCC_VERSION=$(gcc -v 2>&1|tail -1|sed 's/ (GCC) //')

GPP_VERSION=$(g++ -v 2>&1|tail -1|sed 's/ (GCC) //)

GFORTRAN_VERSION=$(gfortran -v 2>&1|tail -1|sed 's/ (GCC) //)
...
echo "            \"version\": \"\","
echo "            \"version_arg\": \"-v\","
echo "            \"version_regex\": \"version (\\\\S+\\\\s*\\\\S*) \\\\(\","
