...
################################################################################
def writeJSON(json_dict: Dict, json_path: FilePath, file_text: str = "", conf_file_name: str = "") -> None:
    ...
    json_dict["generated_at"] = datetime.datetime.now(
    ...
    _logger.warning(
        "Writing {text} JSON configuration file \"{file}\"".format(text=tmp_text, file=json_path))

    with io.open(json_path, mode="w",) as json_file:
        try:
            json.dump(obj=json_dict, fp=json_file, skipkeys=True, indent=4)
        except Exception as excp:
            _logger.critical("error \"{error}\" trying to write {text} JSON configuration to file \"{file}\""
                             .format(error=excp, text=tmp_text, file=json_path))
            sys.exit(EXT_ERR_WR_FILE)

################################################################################
def readJSON(json_path: FilePath, file_text: str = "", conf_file_name: str = "") -> object:
    ...
    """
    _logger.warning("Parsing {text} config file \"{path}\"".format(text=file_text,
        path=json_path))
    ...
    try:
        ...
        _logger.critical("error \"{error}\" parsing file \"{path}\"".format(
                error=exp, path=json_path))
    ...
    try:
        if conf_file_name != "":
            if ret_val.file_name != conf_file_name:
                ...
                _logger.critical("the value of 'file_name' should be \"{should}\" but is \"{but_is}\""
                         .format(should=conf_file_name, but_is=ret_val.file_name))
        ...
        if file_major < CFG_VERSION.major or file_minor < CFG_VERSION.minor:
            _logger.critical("project file \"{path}\" is not a valid project file!".format(
                path=json_path))
            _logger.critical("project file version (the value of 'file_version') is too old. is \"{old}\" should be \"{new}\""
                                  .format(old=ret_val.file_version, new=".".join(CFG_VERSION)))
            sys.exit(EXT_ERR_NOT_VLD)

    except Exception as excp:
        _logger.critical("error \"{error}\" parsing file \"{path}\", JSON file not valid".format(
